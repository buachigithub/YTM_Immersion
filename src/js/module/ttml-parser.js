...
(function (global) {
  ...
  const getAttr = (el, localName, nsUri) => {
    ...
    // Common prefixes
    for (const prefix of ['ttm:', 'tts:', 'tt:', 'itunes:']) {
  ...
  const parseTTML = (xmlString) => {
    ...
    pElements.forEach((p) => {
      ...
      if (pAlign === 'right' || pAlign === 'end') isRight = true;
      if (pAlign === 'center') isCenter = true;
  ...
  if (typeof window !== 'undefined') {
    window.TTMLParser = TTMLParser;
  }
  if (typeof globalThis !== 'undefined') {
    globalThis.TTMLParser = TTMLParser;
  }
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = TTMLParser;
  }
